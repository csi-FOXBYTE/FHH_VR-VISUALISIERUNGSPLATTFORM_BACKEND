...
def log_stderr(msg):
    """Logs auf STDERR (stört JSON nicht)."""
...
def main():
    ...
    if not stype:
        emit_json({"status": "fatal_error", "error": "Service detection failed", "progress": 0.0})
        sys.exit(1)
    ...
    total = len(tasks)
    if total == 0:
        emit_json({"event": "finish", "progress": 1.0, "message": "Nothing to do"})
        sys.exit(0)

    # Start Event
    emit_json({"event": "start", "total_tiles": total, "service_type": stype})
...
if __name__ == "__main__":
    try:
        ...
        emit_json({"status": "fatal_error", "error": str(e)})
        sys.exit(1)