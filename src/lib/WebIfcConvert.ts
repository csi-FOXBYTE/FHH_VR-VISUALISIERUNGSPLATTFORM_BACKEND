...
export async function buildDocumentFromIfc(
  ...
  // A view, not a copy: new Uint8Array(buffer) would duplicate the whole file,
  // which for a 696 MB model is 1,4 GB before a single triangle exists.
  const modelID = api.OpenModel(
    new Uint8Array(bytes.buffer, bytes.byteOffset, bytes.byteLength),
    {
      // web-ifc's own re-origining shifts ALL THREE axes. Measured: a model
      // whose ground floor belongs at 0 m came out 2.50 m too high. Horizontal
      // centring happens further down instead, which leaves height alone.
  ...
  try {
    ...
    // Keyed by geometry alone while merging, because the merge path reads only
    // positions and indices and paints per bucket. Without merging the material
    // sits on the cached primitive, so the colour has to be part of the key -
    // otherwise every reuse of a geometry inherits the first placement's colour.
    const meshCache = new Map<string, ReturnType<Document["createMesh"]>>();
    const meshKey = (geometryId: number, colour: { x: number; y: number; z: number; w: number }) =>
      options.mergeInto ? String(geometryId) : `${geometryId}|${colourKey(colour)}`;
    ...
    const flushBucket = (key: string) => {
      ...
      // A bucket can collect vertices without ever getting indices, when web-ifc
      // hands over a geometry whose index array is empty. Such a bucket must
      // still be dropped: leaving it behind kept flushLargest() selecting it
      // forever without ever reducing heldBytes - one core at 100 %, memory
      // perfectly flat, the job never finishing.
    ...
    const flushLargest = () => {
      ...
      // Report whether the map actually shrank, so the caller's loop cannot spin
      // if flushBucket ever bails out again.
    ...
    api.StreamAllMeshes(modelID, (flatMesh) => {
      ...
      for (let i = 0; i < geometryCount; i++) {
        ...
        const cacheKey = meshKey(placed.geometryExpressID, placed.color);
        let mesh = meshCache.get(cacheKey);
    ...
    // Released even when the conversion throws. BullMQ hands this child
    // process to the next job, and web-ifc keeps its arena - up to
    // MEMORY_LIMIT_MB - allocated until the model is closed. A failed
    // conversion would otherwise poison every job that follows it.
